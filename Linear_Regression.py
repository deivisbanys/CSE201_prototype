...
"""
Created on Thu Dec  6 11:40:23 2018
...
class LinearRegression:
    def __init__(self,features,ratings,alpha = 0.0001 ,max_iterations = 500000, max_error = 0.1, rating_accurary = 2):
...
def generate_ranks(no_features, no_ppl = 10):
    ranks = []
    for i in range(no_features):
        choises = [0,1,2,3,4,5] + [None] * 15
        person = []
        for i in range(no_ppl):
            choice = random.choice(choises)
            person.append(choice)
        if any(k != None for k in person):
            ranks.append(person)
    return ranks
    
def generate_features(no_features,no_papers):
    return [[random.randint(0,1) for _ in range(no_features)] for _ in range(no_papers)]

no_features = 10
no_ppl = 20
no_papers = 30

features = generate_features(no_features, no_papers)
rankings = generate_ranks(no_features,no_ppl)
